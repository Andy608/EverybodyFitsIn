...
public class PieceScript : MonoBehaviour
{
    ...
    public GameObject boundsObj; //Parent gameobject of the piece
    public GameObject artObj; //Art of the piece
    ...
    public void mouseDown(bool isMouseDown)
    {
        ...
        if (isMouseDown)
        {
            soundManager.playFx("small-giggle");
            artObj.GetComponent<SpriteRenderer>().sortingOrder = 3;
    ...
    public void setBig(bool makeBig)
    {
        //Debug.Log(isSmall + " | " + makeBig);
        if (isSmall && makeBig)
        {
            boundsObj.transform.localScale = bigSize;

            if (gameObject.tag == "Rectangle Piece")
            {
                Vector3 artScale = artObj.transform.localScale;
                artScale.x = bigSize.y;
                artScale.y = bigSize.y;
                artObj.transform.localScale = artScale;
            }
            else
            {
                artObj.transform.localScale = bigSize;
            }

            isSmall = false;
        }
        else if (!isSmall && !makeBig)
        {
            Vector3 scale = boundsObj.transform.lossyScale;
            scale = smallSize;
            ...
            {
                Vector3 artScale = artObj.transform.localScale;
                artScale.x = smallSize.y;
                artScale.y = smallSize.y;
                artObj.transform.localScale = artScale;
            }
            else
            {
                artObj.transform.localScale = smallSize;
            }
